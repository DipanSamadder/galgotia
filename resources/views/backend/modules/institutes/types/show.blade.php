    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="body">
                <div class="row">
                <div class="col-lg-6">
                    @if(dsld_have_user_permission('institute_type_add') == 1)
...
@section('footer')
    @include('backend.modules.institutes.types.add')
    ...
    <div class="modal fade" id="edit_larger_modals" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <form id="update_form" action="{{ route('institute_types.update') }}" method="POST" enctype="multipart/form-data" >
                @csrf 
...
<script>
    function add_new_lg_modal_form(){
        ...
        $('#add_new_larger_modals_tile').text('Add New Institute Type');
    }

    function edit_lg_modal_form(id){
        $('#edit_larger_modals_body').html('');
        $('#edit_larger_modals').modal('show');
        $('#edit_larger_modals_title').text('Edit Institute Type');
        $.ajax({
            url: "{{ route('institute_types.edit') }}",
            type: "post",
            cache : false,
            data: {
                    '_token':'{{ csrf_token() }}',
    ...
    function get_pages(){
        ...
        $.ajax({
            url: "{{ route('ajax_institute_types') }}",
            type: "post",
            cache : false,
            data: {
                    '_token':'{{ csrf_token() }}',
