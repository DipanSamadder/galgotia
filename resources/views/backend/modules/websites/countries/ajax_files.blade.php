            ...
            @foreach($data as $key => $value)
                ...
                <tr>
                    ...
                    <div class="custom-control custom-switch">
                        <input type="checkbox" class="custom-control-input" id="status_institute_type{{$value->id }}" onchange="DSLDStatusUpdate('{{ $value->id }}','{{ $value->status == 1 ? 0 : 1  }}', '{{ route('countries.status') }}','{{ csrf_token() }}')" @if($value->status == 1) checked @endif >
                    ...
                    <td>
                            ...
                            <a href="javascript:void(0);" class="btn btn-default waves-effect waves-float btn-sm waves-red bg-danger" onclick="DSLDDeleteAlert('{{ $value->id }}','{{ route('countries.destory') }}','{{ csrf_token() }}')">
